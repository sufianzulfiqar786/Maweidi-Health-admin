...
const PharmacyOrderList = ({ searchQuery }) => {
  ...
  useEffect(() => {
    orderlistPost?.postData(
      `${process.env.REACT_APP_GET_GET_ORDERS}?per_page=${rowsPerPage}&page=${page}&pharmacy=${0}`,
      {},
      (res) => {
            ...
            >
              {!orderlistPost?.isLoading ? visibleRows?.map(({
                ...
                >
                  ...
                  <TableCell align="left">
                    <CardHeader
                      ...
                      avatar={
                        ...
                        >
                          <Avatar alt={name} src={`${process.env.REACT_APP_IMAGE_URL + '/' + user?.profile_pic}`} />
                        </Box>
                      }
                  ...
                  <TableCell align="center">{order_items[0]?.product?.title}</TableCell>
                  <TableCell align="center">{total_amount}</TableCell>
                  <TableCell align="center">
                    <button
                      onClick={() => {
                        navigate(`/medical/equipment/detail/${id}`);
                      }}
